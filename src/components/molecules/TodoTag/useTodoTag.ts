...
export function useTodoTag({ projectId, id }: { projectId: string; id: string; }): Props {
  ...
  return {
    ...
    onUpdateTodo: () => {},
    editTag(todoRef: firebase.firestore.DocumentReference<Todo> | undefined, cb: () => void) {
      if (!todoRef) {
    ...
    deleteTag(todoRef: firebase.firestore.DocumentReference<Todo> | undefined, cb: () => void) {
